...
async function addToWallet(userId, amount) {
  try {
    ...
    console.error("addToWallet error:", e.message);
  }
}
...
async function deductFromWallet(userId, amount) {
  ...
  try {
    ...
    const realAvailable = Math.max(0, totalEarnings - totalWithdrawn);
    if (realAvailable < amount) {
      await conn.rollback();
      return false;
    }

    const newBalance = realAvailable - amount;
    await conn.query(`UPDATE users SET wallet_balance = ? WHERE id = ?`, [newBalance, userId]);
    await conn.commit();
    return true;
  } catch (e) {
    await conn.rollback();
    console.error("deductFromWallet error:", e.message);
    return false;
  } finally {
    conn.release();
  }
